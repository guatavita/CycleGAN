...
def return_generator(base_path, image_keys=('image', 'annotation'), interp_keys=('bilinear', 'nearest'),
    ...
    if ct_clip:
        base_processors += [
            Threshold_Images(image_keys=image_keys, lower_bounds=(-1000,), upper_bounds=(1500,), divides=(False,))]

    if not is_validation:
        tf_path = [os.path.join(base_path, 'train', image_keys[0])]
    else:
        tf_path = [os.path.join(base_path, 'validation', image_keys[0])]
    ...
    if not is_validation:
        ...
        if shuffle:
            processors += [{'shuffle': int(0.15*len(generator))}]
    else:
        processors += [Cast_Data(keys=image_keys, dtypes=dtype_keys),
    ...
    if prep_tfrecord_cache:
        if not is_validation:
            cache = glob.glob(os.path.join(base_path, 'tf_cache', model_desc, 'train', image_keys[0],
                                           '*cache.tfrecord*'))
        else:
            cache = glob.glob(os.path.join(base_path, 'tf_cache', model_desc, 'validation', image_keys[0],
                                           '*cache.tfrecord*'))
        if not cache:
